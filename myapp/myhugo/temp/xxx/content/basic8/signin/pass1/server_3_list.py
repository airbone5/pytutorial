...
from flask import Flask, abort, redirect, render_template, session, url_for,flash # 新增flash
import dotenv
import os
...
def listfiles():
    if "user" not in session:
        abort(404)
    if '@nkust.edu.tw' in session['user']['userinfo']['email'] :
        files=os.listdir('.')
        return render_template("filelist.html",options=files)
    else:
        abort(404)
