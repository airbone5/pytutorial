...
knitr::knit_hooks$set(
  output = function(x, options) {
    ...
    # 好像要檢查兩部分,一個是CHUNK(collapse=T),一個OUTPUT(collapse=F)
    # 如果是T,不會到這裡
    # 但是statamarkdown 處理output就可以了,好像是collapse=F
...
knitr::knit_hooks$set(
  warning = function(x, options) {
    ...
    if (isTRUE(options$hugomd) & isTRUE(options$messages)) {
...
knitr::knit_hooks$set(
  chunk = function(x, options) {
    ...
    print(paste0("chunk results為",options$results))
    if (isTRUE(options$hugomd) && options$results!="asis") { # 只有這個狀態才處理
      if (isTRUE(options$collapse)) { 
        # collapse=T表示輸出輸入都同一個BLOCK,所以把凡是```\n```....?lang=R...\n開頭的都殺掉(就是區塊結尾+區塊開頭)
      ...
      if(tolower(options$engine)=="stata"){
        y<-gsub("[.]\\s*hugocmd.*\n","",y,perl=T) # 例如 . hugocmd attachtoa  就是不讓hugocmd出現在輸出中
        if(isTRUE(options$cuthugo)){
          #將attachtoa巨集放在chunk外面
          chunkhead<-chunkPrefix(options,"output")
          y<-gsub("(\\{\\{<\\s+attachtoa\\s+.*?>\\}\\})",paste0("\n```\n","\\1","\n\n",chunkhead,"\n"),y)
          #print(y)
          y<-gsub("\\n\\n([`]{3,})\\S+\\?[^\\n]*?\\n\\n\\1","\n",y,perl=T) #如果attachtoa在最後一行會產生了一個CHUNK,裡面是空的
        }  
      }
      # 只適用於expand
