...
config_stata(r"c:\program files\stata17\StataMP-64.exe", force_batch=True)
# ## Configure ipystata
...
get_ipython().run_cell_magic('stata', '-s mata_session', 'sysuse auto')


# In[19]:

get_ipython().run_cell_magic('stata', '--mata -s mata_session', 'y    = st_data(., "price")\nX    = st_data(., "mpg trunk")\nn    = rows(X)\nX    = X,J(n,1,1)\nXpX  = quadcross(X, X)\nXpXi = invsym(XpX)\nb    = XpXi*quadcross(X, y)\nb\'')
